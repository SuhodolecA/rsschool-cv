## Anton Sukhadolets

***Contact Info***: 
- tel: +375333130021;
- email: Kut18@tut.by;
- linkedin: https://www.linkedin.com/in/anton-suhodolec-276283a3/;

***Summary***:
Hi, my name is Anton and I like web programming. I don't have much experience, but I try to get new knowledge every day.
My strengths are:
- composure (don't panic no matter what happens)
- flexibility
- honesty
- continuous learning (self-development)
- appreciate the time

I guess my weaknesses are:
- self-criticism
- too detail oriented
- too kind
 
In my learning I use such online resources as:
* codecademy ([link to my profile](https://www.codecademy.com/profiles/design6714341631))
* codewars ([link to my profile](https://www.codewars.com/users/Ant11))
* freecodecamp ([link to my profile](https://www.freecodecamp.org/fccd05e9812-e2ea-4038-920a-15d1b3799051))
* code-basics.com
and some others...

In addition to online learning, I like to read books. Now I am reading:
* Keith J. Grant - CSS in Depth
* Nick Morgan - JavaScript for Kids: A Playful Introduction to Programming
* Haverbeke M. - Eloquent JavaScript

I also watch tutorial videos on youtube and play games like:
- CSS Diner ([link to the game](https://flukeout.github.io/))
- FLEXBOX FROGGY ([link to the game](https://flexboxfroggy.com/))
- GRID GARDEN ([link to the game](https://cssgridgarden.com/))

During self-education I met and learned how to apply the following technologies:
***Skills***: 
* HTML(including semantic tags)
* CSS(including animations, flexbox, css grid and css variables)
* JavaScript
* jQuery(including slick slider)
* React.js(basic), 
* LESS
* Git
* BEM methodology
* Gulp 4(basic)
* Node.js(basic)
* Npm(basic)


***Experience***: No production experience in web programming, but try to learn something new every day.
Some training projects that I did:
- Advocatus (landing page for a lawyer agency) [Link to the page](https://suhodoleca.github.io/Advocatus-project/);
- Simple dropdown menu [Link to the project](https://suhodoleca.github.io/Dropdown-Menu/);
more in my github [Link to the github](https://github.com/SuhodolecA?tab=repositories)

***Education***: 
* October 2019 - February 2020 IT SPECIALISTS TRAINING CENTER SkillUP, FRONT-END DEVELOPING.
* 2012-2013 Institute for Advanced Qualifications and Retraining BNTU, Logistics, Logistician - economist.
* 2007-2012 Belarusian National Technical University, Instrument making, Electrician - engineer.
* 2007 Finished high school №180

***English***:  A2+
